...
- バイアス
    - 0 にすることも多い
    - 0 にしない場合、周辺統計量を使うとよい場合が多い

+++
- 教師なし学習での学習パラメータを利用する
- 関連あるタスクでの教師あり学習を行った学習パラメータを利用する
...
- 凸最適化では望ましい理論的な性質を持つ
    - 解説https://qiita.com/Quasi-quant2010/items/0c50515a296be2dcb3ef
    - ミニバッチの場合とバッチの場合で更新量の差異を減らしたらしい
...
- 二次近似まで用いる最適化手法による DNN のトレーニングについて議論する
- 正直あまり使っている話はあまりきかない
    - 計算量とかメモリ使用量とかがきつい
...
- 鞍点ではニュートン法が正常に動作しない
    - ヘッセ行列が整定値にならないため
    - 修正手法としてレーベンバーグ・マーカート法 など
- 計算量
    - 更新のたびに逆ヘッセ行列が必要なため現実的でない

+++
### 8.6.2 Conjugate Gradients
- 共役勾配を用いると逆ヘッセ行列の計算を回避可能
    - 共役勾配法で検索
- DNN では非線形共役勾配法を用いる
    - 目的関数が(多分)非線形の場合に利用する変形手法

+++
### 8.6.3 BFGS
- 逆ヘッセ行列の近似行列により計算を効率化
    - 更新幅の探索は共役勾配法より高速
    - メモリ使用量が $O(n^2)$ なので利用は難しい
- 省メモリ手法も提案されている
    - 近似行列の保持をさぼる
    - メモリ使用量が $O(n)$ まで改善
---
...
- (1つの)谷の周りを振動している場合、平均すると良い解になる
    - 凸最適化の場合算術平均
    - 非凸最適化の場合指数移動平均
        - 過去は全然違うところにいるかもしれないので指数的に影響力を下げる
+++
...
- FitNets
    - 日本語の記事はほぼない
    - 教師ネットワークの(出力層だけでなく)中間層も模倣するように生徒ネットワークを学習する

+++
### 8.7.5 Designing Models to Aid Optimization
- 最適化が簡単になるようなモデル設計をすることで最適化が改善されることも多い
- 最適化が容易なモデルを選択する
- ネットワーク構造を工夫する
    - スキップ接続
...
- Continuation Methods
    - コスト関数を簡単なものから難しいものに変えて学習実施
    - 極小値を避けて大域最適解に近づくよう誘導するイメージ
- カリキュラムラーニング
    - 簡単なものから順に学習する
    - RNN の学習に効果がある
